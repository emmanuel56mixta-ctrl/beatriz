...
const q=(n:number)=>n===1?"1":n===4?"4B":"8B";
const cs=(s:CellStatus,b:number|null)=>s==="ARMED"?`ARM ${b??""}`:s==="DISARMING"?`OUT ${b??""}`:s==="BUILDING"?`BUILD ${b??""}`:s;
const es=(s:ExtraStatus,b:number|null)=>s==="ARMED"?`ARM ${b??""}`:s==="EXIT"?`OUT ${b??""}`:s;
...
function Knob({label,value,min=0,max=1,onChange,format}:{label:string;value:number;min?:number;max?:number;onChange:(v:number)=>void;format?:(v:number)=>string}){
  ...
  function move(e:PointerEvent<HTMLButtonElement>){const d=drag.current;if(!d||d.id!==e.pointerId)return;e.preventDefault();e.stopPropagation();const next=Math.max(min,Math.min(max,d.v+(d.y-e.clientY)*(max-min)/115));onChange(next);}
  function up(e:PointerEvent<HTMLButtonElement>){if(drag.current?.id===e.pointerId)drag.current=null;e.stopPropagation();}
  return <label className="mini-knob"><button type="button" className="knob-face" style={{"--knob-angle":`${angle}deg`} as React.CSSProperties} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={up}><i/></button><span>{label}</span><b>{format?format(value):Math.round(value*100)}</b></label>;
}
...
export function AudioLabBuilderV308(){
  ...
  useEffect(()=>{let raf=0;const draw=()=>{const canvas=canvasRef.current;if(canvas){const dpr=Math.min(2,window.devicePixelRatio||1),w=Math.max(1,canvas.clientWidth),h=Math.max(1,canvas.clientHeight);if(canvas.width!==Math.round(w*dpr)||canvas.height!==Math.round(h*dpr)){canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);}const g=canvas.getContext("2d");if(g){g.setTransform(dpr,0,0,dpr,0,0);g.clearRect(0,0,w,h);g.fillStyle="#080b0b";g.fillRect(0,0,w,h);g.strokeStyle="rgba(255,255,255,.055)";g.lineWidth=1;for(let i=1;i<8;i++){const x=w*i/8;g.beginPath();g.moveTo(x,0);g.lineTo(x,h);g.stroke();}for(let i=1;i<4;i++){const y=h*i/4;g.beginPath();g.moveTo(0,y);g.lineTo(w,y);g.stroke();}
      const a=cellRef.current?.spectrum()??new Uint8Array(),b=extraRef.current?.spectrum()??new Uint8Array(),bars=72,top=h*.66;for(let i=0;i<bars;i++){const p=i/(bars-1),idx=Math.floor(Math.pow(p,2.15)*Math.max(0,Math.max(a.length,b.length)-1)),av=a[idx]??0,bv=b[idx]??0,v=Math.min(255,av*.82+bv*.72),bh=(v/255)*(top-8),x=i*w/bars,bw=Math.max(1,w/bars-1);g.fillStyle=`rgba(129,210,181,${.18+.7*v/255})`;g.fillRect(x,top-bh,bw,bh);}
      const wa=cellRef.current?.waveform()??new Uint8Array(),wb=extraRef.current?.waveform()??new Uint8Array();g.strokeStyle="rgba(238,199,143,.9)";g.lineWidth=1.4;g.beginPath();const y0=h*.82;for(let x=0;x<w;x++){const p=x/Math.max(1,w-1),ia=Math.floor(p*Math.max(0,wa.length-1)),ib=Math.floor(p*Math.max(0,wb.length-1)),v=(((wa[ia]??128)-128)+((wb[ib]??128)-128)*.8)/128,y=y0-v*h*.12;if(x===0)g.moveTo(x,y);else g.lineTo(x,y);}g.stroke();g.fillStyle="rgba(200,198,190,.55)";g.font="7px ui-monospace,monospace";["60","250","1K","4K","16K"].forEach((t,i)=>g.fillText(t,4+i*(w-24)/4,h-5));}}
  ...
  const cellTile=(d:(typeof CELL_DEFINITIONS)[number])=>{const rt=cells.layers[d.id],busy=rt.status==="ARMED"||rt.status==="DISARMING"||rt.status==="BUILDING";return <article className={`channel-tile origin-cell state-${rt.status.toLowerCase()}`} key={d.id}><button type="button" className="channel-trigger" disabled={!cells.running||busy} onClick={e=>{armVisual(e.currentTarget,d.id);cellRef.current?.toggle(d.id)}}><span><i>{q(d.q)}</i><b>{cs(rt.status,rt.targetBar)}</b></span><strong>{d.label}</strong></button>{controls(d.id)}</article>;};
  const extraTile=(d:(typeof EXTRA_DEFINITIONS)[number])=>{const rt=extras.layers[d.id],busy=rt.status==="ARMED"||rt.status==="PLAYING"||rt.status==="EXIT";return <article className={`channel-tile origin-${d.kind.toLowerCase()} state-${rt.status.toLowerCase()}`} key={d.id}><button type="button" className="channel-trigger" disabled={!cells.running||busy} onClick={e=>{armVisual(e.currentTarget,d.id);extraRef.current?.toggle(d.id)}}><span><i>{q(d.q)}</i><b>{es(rt.status,rt.targetBar)}</b></span><strong>{d.label}</strong></button>{controls(d.id)}</article>;};
  ...
  return <main className="mixer308-shell">
    ...
    <footer className="mixer308-status"><i className={cells.running?"on":""}/><b>{cells.running?"RUN":"STOP"}</b><p>{msg}</p><span>GAIN · CUT · Q · SEND · LP/BP/HP PER CHANNEL</span></footer>
  </main>;
}

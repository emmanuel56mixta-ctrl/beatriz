...
export type LayerId = CellLayerId | LiveLayerId;
...
export const LAYER_DEFINITIONS: LayerDefinition[] = [
  ...
  { id:"BUILD", label:"BUILD → DROP", description:"4 compases reales de subida; rompe con CLUB + HOOK.", quantizeBars:4, kind:"build", origin:"CELL", cells:[{source:"matrodaDrums",bar:48,bars:4,gain:.82},{source:"cityOther",bar:72,bars:4,gain:.24}] },
...
export class QuantizedLayerBuilderV303 {
  ...
  private liveStarts=new Map<LiveLayerId,number>();
  private liveStops=new Map<LiveLayerId,number>();
  private onState:(state:BuilderState)=>void;

  state:BuilderState={ready:false,loading:false,running:false,bar:1,beat:1,phase:0,message:"Pulsa START. BASE corre sola hasta que tú añadas textura.",error:null,layers:blankLayers()};
  ...
  async prepare(){
    ...
    this.emit({loading:true,error:null,message:"Cargando células reales y preparando instrumentos LIVE…"});
    try{
      ...
      this.emit({ready:true,loading:false,message:"Listo. Los LIVE INSTRUMENTS no usan MP3."});
  ...
  async start(){
    ...
    this.startClock();this.startLiveScheduler();this.emit({message:"BASE ON. Ahora añade textura LIVE o células reales cuando quieras."});
  }
  ...
  private activateAt(def:LayerDefinition,when:number){
    if(def.kind==="live"){this.liveStarts.set(def.id as LiveLayerId,when);this.liveStops.delete(def.id as LiveLayerId);return;}
  ...
  private armOff(def:LayerDefinition){const targetBar=this.nextBoundary(def.quantizeBars);const when=this.timeForBar(targetBar);this.patchLayer(def.id,{status:"DISARMING",targetBar:targetBar+1});this.emit({message:`${def.label} EXIT · BAR ${targetBar+1}.`});
    if(def.kind==="live"){this.liveStops.set(def.id as LiveLayerId,when);this.at(when+.03,()=>{this.liveStarts.delete(def.id as LiveLayerId);this.liveStops.delete(def.id as LiveLayerId);this.patchLayer(def.id,{status:"OFF",targetBar:null});});return;}
  ...
  private armBuild(def:LayerDefinition){const targetBar=this.nextBoundary(def.quantizeBars);const when=this.timeForBar(targetBar);const end=when+4*BAR_SECONDS;this.patchLayer("BUILD",{status:"ARMED",targetBar:targetBar+1});this.emit({message:`BUILD ARMED · BAR ${targetBar+1}.`});this.fadeHandles(this.currentDrumHandles(),.22,when,.14);const oneShots=def.cells.map(cell=>this.startOneShot(cell,when));
    this.at(when,()=>{if(!this.state.running)return;this.patchLayer("BUILD",{status:"BUILDING",targetBar:targetBar+5});this.emit({message:"BUILDING · 4 compases → DROP."});});
    this.at(end,()=>{if(!this.state.running)return;oneShots.forEach(h=>{try{h.source.stop();}catch{}});this.patchLayer("BUILD",{status:"OFF",targetBar:null});this.forceDropFlavor(end);this.emit({message:`DROP · CLUB + HOOK · BAR ${targetBar+5}.`});});
  }
  ...
  private liveAt(id:LiveLayerId,time:number){const start=this.liveStarts.get(id);if(start==null||time<start-.001)return false;const stop=this.liveStops.get(id);return stop==null||time<stop-.001;}

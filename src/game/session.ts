...
import { CoreAudio, type SectionVariant, type SkinStage, type TensionState } from "./coreAudioV204";
...
export class Session{
  ...
  private apply(events:GameEvent[]){if(!events.length)return;for(const ev of events){if(ev.kind==="harddrop")this.juice.addTrauma(.14);if(ev.kind==="lock"){this.house.lock(ev.piece);this.juice.addTrauma(.08);}if(ev.kind==="clear"){
      this.house.clear(ev.lines,ev.combo,this.engine.lines,ev.rowPatterns??[]);
      const label=ev.lines===4?"TETRIS":ev.lines===3?"TRIPLE":ev.lines===2?"DOUBLE":"SINGLE";this.flash(ev.lines===4?"TETRIS · NEW KIT":`${label} · MUSIC BUILT`,ev.lines>=4?1100:750);this.juice.addTrauma(.16+ev.lines*.09);const color=tokenColor(ev.tspin?"T":"I");for(const row of ev.clearedRows)for(let c=0;c<10;c++){const{x,y}=this.renderer.cellCenter(c,row);this.juice.burst(x,y,color,3);}const mid=this.renderer.cellCenter(5,10);this.juice.float(mid.x,mid.y,label);
    }if(ev.kind==="gameover"){this.mode="over";this.input.enabled=false;this.house.tapeStop();const hi=Math.max(this.hud.high,this.engine.score);saveHigh(hi);this.hud.high=hi;this.flash("FIN",1600);}}this.emit();}
  private flash(text:string,ms:number){this.flashText=text;this.flashUntil=performance.now()+ms;}
  private syncHud(){const snap=this.engine.snapshot();const clock=this.house.visual();const mix=analyze(this.engine,this.house.energy);const nextMomentum=(Math.floor(this.house.momentum/4)+1)*4;this.hud={score:snap.score,high:Math.max(this.hud.high,snap.score),lines:snap.lines,level:snap.level,combo:snap.combo,tetrises:snap.tetrises,bpm:clock.bpm,bar:clock.bar,beat:clock.beat,step:clock.step,boardHeight:mix.stackHeight,skinStage:this.house.skinStage,skinName:clock.phrase.split(" · ")[1]??"SKELETON",section:this.house.sectionVariant,tension:this.house.tensionState,lastGesture:this.house.lastGesture,flash:this.flashText,momentum:this.house.momentum,nextMomentum,memoryCount:this.house.memoryCount,inventoryName:this.house.inventoryName,instrumentTier:this.house.instrumentTier,instrumentName:this.house.instrumentName,lastRowSignature:this.house.lastRowSignature,hold:snap.hold,canHold:snap.canHold,next:snap.next};this.onHud(this.hud,this.mode);}

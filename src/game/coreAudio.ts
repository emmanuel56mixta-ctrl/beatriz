...
export class CoreAudio {
  ...
  // Earned macro reward. A TETRIS creates one bar of air, then ten bars of
  // a curated vocal spotlight (~19.35 s at 124 BPM). The vocal never loops.
  ...
  async unlock() {
    ...
    // Do not block START CORE while the private vocal downloads.
  ...
  private scheduleStep(absStep: number, time: number) {
    ...
    // TETRIS is the one earned exception to the sacred skeleton: one finite bar
    // of air. It cannot become a long breakdown or a background playback mode.
    if (rewardBreak) {
      if ([4, 8, 12].includes(step)) this.rim(time, 0.055, 1350 + step * 70);
      if (step >= 13) this.rim(time, 0.07 + (step - 13) * 0.025, 2100 + (step - 13) * 650);
      if (step === 15) this.sweepNoise(time, 0.17, 0.34);
    ...
    const danger = this.tensionState;

    if (stage === 0) {
      if (step === 2 || step === 10) this.hat(time, 0.025, false);
    } else {
      if ([2, 6, 10, 14].includes(step)) {
        this.hat(time, stage >= 3 ? 0.075 : 0.055, stage >= 3 && (step === 6 || step === 14));
      }
    ...
    // Finite danger build: density grows for four bars and then latches to a
    // tense but stable groove. No continuous riser or filter drone.
    if (danger === "BUILD") {
      const buildBar = Math.max(0, bar - this.dangerBuildStartBar);
      if (step % 2 === 1 && step < 15) this.hat(time, 0.012 + buildBar * 0.008, false);
      if (buildBar >= 2 && (step === 5 || step === 13)) this.rim(time, 0.035 + buildBar * 0.008, 1900);
      if (buildBar === 3 && step >= 12) this.rim(time, 0.04 + (step - 12) * 0.018, 1700 + step * 80);
    } else if (danger === "TENSE" && (step === 7 || step === 15)) {
      this.ghost(time, 0.055);
    }

    this.scheduleBass(step, time, stage, variant, rewardVocal);
    this.emitStep(step, time);
  }

  private emitStep(step: number, time: number) {
    if (!this.onStep || !this.ctx) return;
    const delay = Math.max(0, (time - this.ctx.currentTime) * 1000);
    window.setTimeout(() => this.onStep?.(step, time), delay);
  }

  private scheduleBass(step: number, time: number, stage: SkinStage, variant: SectionVariant, vocalActive = false) {
    const scale = vocalActive ? 0.82 : 1;
    if (stage <= 1) {
      if (step === 0 || step === 8) {
        this.bass(time + 0.028, step === 0 ? 55 : 65.41, (stage === 0 ? 0.13 : 0.14) * scale, 0.16);
      }
      return;
    }
    ...
    if (hit) this.bass(time + (step % 4 === 0 ? 0.03 : 0), hit[1], (stage >= 4 ? 0.18 : 0.15) * scale, 0.14);
  }
  ...
  private kick(t: number, amp: number) {
    ...
    click.type = "triangle";
    click.frequency.setValueAtTime(1450, t);
    click.frequency.exponentialRampToValueAtTime(520, t + 0.025);
  ...
  private duckBass(t: number) {
    const bus = this.bassBus;
    if (!bus) return;
    bus.gain.cancelScheduledValues(t);
    bus.gain.setValueAtTime(Math.max(0.15, bus.gain.value), t);
    bus.gain.linearRampToValueAtTime(0.24, t + 0.004);
    bus.gain.exponentialRampToValueAtTime(1, t + 0.085);
  }
  ...
  private iGesture(t: number) {
    ...
    bp.frequency.exponentialRampToValueAtTime(5200, t + 0.17);
    bp.Q.value = 1.5;
    ...
    src.connect(bp).connect(pan).connect(g).connect(this.master);
    src.start(t);
    src.stop(t + 0.2);
  }

  private tGesture(t: number) {
    const ctx = this.ctx;
    if (!ctx || !this.master) return;
    const pan = ctx.createStereoPanner();
    pan.pan.value = 0.25;
    const sum = ctx.createGain();
    sum.gain.value = 0.055;
    const lp = ctx.createBiquadFilter();
    lp.type = "lowpass";
    ...
    for (const hz of [220, 261.63, 329.63]) {
  ...
  private sweepNoise(t: number, amp: number, dur: number) {
    ...
    hp.frequency.exponentialRampToValueAtTime(1300, t + dur);
    const g = ctx.createGain();
    g.gain.setValueAtTime(0.0001, t);
    g.gain.linearRampToValueAtTime(amp, t + dur * 0.7);
    g.gain.exponentialRampToValueAtTime(0.0001, t + dur);
    src.connect(hp).connect(g).connect(this.master);
    src.start(t);
    src.stop(t + dur + 0.02);
  ...
  lock(piece: PieceId) {
    ...
    const velocity = 0.07 + Math.min(0.03, (this.stackHeight / 20) * 0.03);
    this.rim(t, velocity, 3400);
    if (piece === "I") {
      this.iGesture(t + 0.006);
      this.lastGesture = "I · SWEEP";
    } else if (piece === "T") {
      this.tGesture(t + 0.006);
      this.lastGesture = "T · STAB";
    } else {
      this.lastGesture = "LOCK · TICK";
    }
  }
  ...
  hardDrop() {
    ...
    if (onOne) {
      this.kick(t, 0.62);
      this.crash(t, 0.16, 0.28);
      this.lastGesture = "HARD DROP · ON THE 1";
    } else {
      this.rim(t, 0.17, 2200);
      this.lastGesture = "HARD DROP · OFF GRID";
    }
    return onOne;
  }
  ...
  clear(lines: number, combo: number, totalLines: number) {
    const ctx = this.ctx;
    if (!ctx || !this.running || lines <= 0) return;
    ...
    if (lines === 1) {
      ...
      this.armVocalReward();
      this.lastGesture = "TETRIS · AIR → VOCAL";
    }
  ...
  private armVocalReward() {
    ...
    // Never stack two vocal files. A back-to-back TETRIS while the spotlight is
    // live still gets its fill/crash but does not create vocal mud.
    if (currentBar >= this.rewardBreakBar && currentBar < this.rewardVocalEndBar) {
      this.lastGesture = "B2B TETRIS · VOCAL ALREADY LIVE";
      return;
    }
  ...
  private startVocalReward(time: number) {
    ...
    // City Of Dreams is 124 BPM, exactly the v0.20 clock. Start at bar 48,
    // where the separated vocal is strongly active, and play ten bars only.
    ...
    gain.gain.linearRampToValueAtTime(0.42, time + 0.08);
    gain.gain.setValueAtTime(0.42, time + Math.max(0.1, duration - 0.5));
    gain.gain.exponentialRampToValueAtTime(0.0001, time + duration);

    source.connect(hp).connect(lp).connect(gain).connect(master);
    source.start(time, sourceOffset, duration);
    source.onended = () => {
      if (this.rewardSource === source) this.rewardSource = null;
    };
    this.rewardSource = source;
  }

  private stopRewardSource() {
    if (!this.rewardSource) return;
    try { this.rewardSource.stop(); } catch { /* already stopped */ }
  ...
  setBoardHeight(rows: number) {
    ...
    if (!this.dangerLatched && rows >= 14) {
      ...
      this.crash(t, 0.08, 0.18);
      this.tGesture(t + 0.015);
      this.lastGesture = "RELEASE · DANGER EXIT";
    }
  }
  ...
  private nextBeatTime(now: number) {
    ...
    if (t < now + 0.015) {
      target += 1;
      t = this.startAt + target * this.beatDuration;
    }
    return t;
  }
  ...
  visual(): VisualClock {
    ...
    const dna = {
      bass: clamp(this.skinStage / 4),
      harmony: 0,
      hook: vocalActive ? 1 : 0,
      groove: clamp(this.skinStage / 4),
      perc: clamp(Math.max(0, this.skinStage - 1) / 3),
      space: rewardBreak ? 1 : 0,
      drive: this.tensionState === "CALM" ? 0.15 : 0.65,
    };
    const production: Production = { gain: this.userVolume, filter: 1, room: 0, delay: 0 };
    const arrangement = rewardBreak ? "break" : this.tensionState === "BUILD" ? "build" : "groove";
    const rewardLabel = rewardBreak ? " · AIR" : vocalActive ? " · VOCAL" : "";
    ...
    return {
      ...
      dna,
      production,
      phrase: `${this.sectionVariant} · ${SKIN_NAMES[this.skinStage]}${rewardLabel}`,
      mood: this.tensionState,
    };
  }

  waveform(): Uint8Array {
    if (!this.analyser || !this.analyserData) return new Uint8Array();
  ...
  requestDrop() {
    if (this.ctx) {
      this.crash(this.nextBeatTime(this.ctx.currentTime), 0.11, 0.18);
      this.lastGesture = "POWER · ACCENT";
    }
  }
  ...
  tapeStop() {
    ...
    this.master.gain.exponentialRampToValueAtTime(0.0001, t + 0.45);
    window.setTimeout(() => this.stop(), 500);
  }
}

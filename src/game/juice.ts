...
export class Juice {
  ...
  addTrauma(n: number) {
    if (this.reduced || !this.enabled) return;
  ...
  float(x: number, y: number, text: string) {
    this.floaters.push({ x, y, text, life: 0.9, max: 0.9 });
  }
  ...
  offset(t: number) {
    if (this.reduced || !this.enabled || this.trauma <= 0) return { x: 0, y: 0, r: 0 };
    const s = this.trauma * this.trauma;
    return {
      x: (hash(t * 17.2) * 2 - 1) * s * 10,
      y: (hash(t * 13.7 + 2) * 2 - 1) * s * 10,
      r: (hash(t * 9.1 + 5) * 2 - 1) * s * 0.018,
    };
  }
}
